...
const guides: Record<"days" | "budget", Guide> = {
  ...
  budget: {
    ...
    sections: [
      {
        label: "COST 00", title: "費用の目安は、ひとつの総額ではなく「条件の違う参考例」として読む",
        body: [
          "「スリランカ旅行の費用」を調べると、一週間の総額や一人あたりの金額がすぐに見つかります。ただし、その数字には出発地、渡航時期、航空券の乗り継ぎ、ホテルの部屋人数、食事の選び方、観光の内容、移動方法が含まれています。数字だけを比べても、自分の旅の予算にそのまま当てはまるとは限りません。",
          "たとえば、2026年の旅行ガイドでは一週間の費用を一人約18万円からとする例があり、別の7泊9日の旅行記では、リゾート滞在や観光を含む実費が約28万円と紹介されています。これはどちらかが正しい・間違いということではなく、旅の条件で予算が大きく動くことを示す二つの参考例です。",
          "この記事では、相場を一つの断定額として扱うのではなく、何にどれだけ配分すると自分の旅が成立するかを考えます。まず航空券とホテル、次に移動、最後に食事・観光・予備費の順で整理すれば、検索結果の金額に振り回されず、自分の条件に合う総額をつくれます。",
        ],
        image: { src: "/manus-storage/sri-lanka-budget-cost-framework_bf0af3a4.png", alt: "航空券・宿泊・移動・観光の費目を整理するスリランカ旅行予算のイメージ", caption: "費用の目安は、旅の条件が違う複数の参考例を見比べてから、自分の費目へ落とし込みます。", ratio: "standard" },
      },
      {
    ...
    references: [{ label: "ランカミー：スリランカタクシーチャーターサービス公式サイト", href: "https://srilankataxicharterservice.com/ja/" }, { label: "NEWT：スリランカ旅行1週間にかかる費用は？節約方法も紹介（2026年7月確認）", href: "https://newt.net/lka/mag-860019270" }, { label: "TRAVELOG：7泊9日で実際にかかった費用の実例（2023年9月公開）", href: "https://travelog.blog/srilankahiyou/" }],
  },
};
