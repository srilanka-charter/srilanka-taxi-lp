 ...
 * These pages use documentary photos and person-free generated replacements as visual waypoints, combining a clear timeline with practical private-car planning guidance.
 */
...
type Day = { label: string; stay: string; title: string; body: string; detail: string[]; note: string; images?: GalleryImage[] };
...
const itineraries: Record<"five" | "seven" | "four", Itinerary> = {
  five: {
    ...
    hero: { src: "/manus-storage/itinerary-5day-generated-sigiriya_748fc45b.png", alt: "朝の光を受けるスリランカの世界遺産シーギリヤロック", caption: "5日間なら、文化三角地帯に滞在拠点を置いて世界遺産をつなぐのが王道です。" },
    intro: [
  ...
  seven: {
    ...
    hero: { src: "/manus-storage/itinerary-7day-generated-galle-hero_01c34206.png", alt: "夕方の光を受けるスリランカ南部の世界遺産ゴール旧市街", caption: "7日間あれば、文化三角地帯・高原・南部海岸を一つの旅につなげられます。" },
    intro: [
    ...
    days: [
      ...
      { label: "DAY 2", stay: "シーギリヤ周辺泊", title: "シーギリヤロックとダンブッラを、暑さに合わせて巡る", body: "朝はシーギリヤロック、午後はダンブッラ石窟寺院へ。二つの訪問順は天候・混雑・体力で入れ替えて構いません。時間に追われないよう、ホテルで休憩する余白も残しましょう。", detail: ["シーギリヤロックは、登頂までの道のりだけでなく、麓から岩山を見上げる時間にも魅力があります。朝のうちに登るプランなら、ホテルに戻って涼しい時間に休憩を入れ、その後にダンブッラへ向かう流れが快適です。", "寺院の見学では、濡れた路面や裸足で歩く場面もあるため、歩きやすい靴と拭けるタオルを用意しておくと安心です。短い旅でも、この日は観光を増やしすぎず、文化三角地帯の中心をじっくり味わいましょう。"], images: [{ src: "/manus-storage/itinerary-7day-generated-sigiriya-day2_e274a2c2.png", alt: "朝の光を受けるシーギリヤロック", caption: "DAY 2：朝のシーギリヤロックは、世界遺産巡りの中心となる時間です。" }, { src: "/manus-storage/dambulla-ceiling_da604173.jpg", alt: "ダンブッラ石窟寺院の仏像と天井画", caption: "DAY 2：午後はダンブッラ石窟寺院で、洞窟の仏像と天井画を見学します。" }], note: "世界遺産を一日にまとめる日は、車に飲み物や着替えを置けると快適です。" },
      { label: "DAY 3", stay: "キャンディ泊", title: "キャンディへ移動し、仏歯寺の夜に触れる", body: "文化三角地帯からキャンディへ移動します。到着後はホテルで少し休み、夕方から仏歯寺周辺を訪れる流れが組みやすいでしょう。寺院の行事・入場・服装などは、当日の案内に従ってください。", detail: ["移動途中の立ち寄りは、スパイスガーデンや食事など、ひとつか二つに絞るのがポイントです。予定を入れすぎると、キャンディ到着が遅れ、ホテルで休む時間や寺院周辺を歩く時間が削られてしまいます。", "仏歯寺を訪れるなら、夕方以降の混雑や参拝の空気も含めて楽しみます。写真だけを急いで撮るのではなく、服装を整え、入場時の案内を確認しながら、キャンディらしい一夜を過ごしましょう。"], images: [{ src: "/manus-storage/itinerary-7day-generated-kandy-day3_3405c1d1.png", alt: "夕方のキャンディ仏歯寺", caption: "DAY 3：キャンディでは、仏歯寺を訪れる時間をその日の中心に置きます。" }], note: "都市間移動の途中にスパイスガーデンや食事を入れる場合は、滞在時間を決めておくと安心です。" },
      { label: "DAY 4", stay: "ヌワラエリヤまたはエッラ泊", title: "高原へ向かい、紅茶列車を旅のハイライトにする", body: "キャンディから高原へ進み、旅程に合う区間で紅茶列車を楽しみます。列車の時刻・予約・運行状況は事前に確認し、駅まで・駅からの移動と荷物の扱いまで考えておくことが大切です。", detail: ["紅茶列車は、都市間を最短で移動するための乗り物ではなく、窓の外に広がる高原の景色を味わうための時間として組み込みます。どの区間に乗るか、席をどうするか、荷物をどう運ぶかまで決めておくと、当日は景色に集中しやすくなります。", "列車を降りた後の駅からホテルまでを、事前に手配した車でつなぐと、暗くなる時間や荷物の心配を小さくできます。列車の運行状況は変わり得るため、必ず公式案内と当日の情報を確認してください。"], images: [{ src: "/manus-storage/itinerary-7day-generated-tea-train-day4_e7763bfc.png", alt: "高原の茶畑を走る紅茶列車", caption: "DAY 4：紅茶列車は、移動を急ぐためではなく高原の景色を味わう時間として組み込みます。" }], note: "列車は景色を味わう時間に。ホテル移動と観光を同時に詰め込みすぎないようにします。" },
  ...
  four: {
    ...
    hero: { src: "/manus-storage/itinerary-4day-generated-sigiriya-hero_0fb8dcc3.png", alt: "朝の光を受けるスリランカの世界遺産シーギリヤロック", caption: "3泊4日では、見どころを絞り、移動の無駄を減らすことが旅を楽しむ鍵です。" },
    intro: [
...
function ItineraryPage({ variant }: { variant: "five" | "seven" | "four" }) {
  ...
  return <div className="min-h-screen" style={{ backgroundColor: "#0A1520", color: "#F0F4F8" }}><MediaHeader activeCategory="itinerary" /><main>
    ...
    <article className="max-w-3xl mx-auto px-5 md:px-8 pb-24 md:pb-32"><div className="border-b py-11 md:py-14 space-y-7 md:space-y-8" style={{ borderColor: "rgba(255,255,255,0.14)" }}>{itinerary.intro.map((paragraph) => <p key={paragraph} className="font-serif-jp text-lg md:text-xl leading-9" style={{ color: "#DCE6EC" }}>{paragraph}</p>)}</div>
      ...
      <section className="mt-16 md:mt-20 space-y-6">{itinerary.related.map((link) => <p key={link.href} className="font-sans text-[15px] leading-8 md:leading-9" style={{ color: "#C7D3DB" }}>{link.before}<Link href={link.href} className="underline decoration-[#E8732A] underline-offset-4 hover:text-white">{link.text}</Link>{link.after}</p>)}</section>
      <Heading label="FAQ">このモデルコースでよくある質問</Heading><div className="divide-y border-y" style={{ borderColor: "rgba(255,255,255,0.14)" }}>{itinerary.faqs.map((faq, index) => <details key={faq.question} className="group py-6"><summary className="cursor-pointer list-none flex items-start gap-4 font-serif-jp font-bold text-white"><span className="font-montserrat text-[10px] tracking-[0.16em] mt-1" style={{ color: "#E8732A" }}>Q{String(index + 1).padStart(2, "0")}</span><span className="flex-1 leading-7">{faq.question}</span><span className="text-xl leading-none transition-transform group-open:rotate-45" style={{ color: "#E8732A" }}>+</span></summary><p className="font-sans text-sm leading-8 mt-5 pl-10" style={{ color: "#C7D3DB" }}>{faq.answer}</p></details>)}</div>
