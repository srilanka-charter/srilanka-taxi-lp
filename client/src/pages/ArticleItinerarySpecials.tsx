 ...
 * These itinerary pages pair a clear day-by-day narrative with real trip photos and person-free generated replacements placed inside the relevant day, not in a separate gallery.
 */
...
const courses: Record<"west" | "tea" | "women" | "yala", Course> = {
  ...
  tea: {
    ...
    hero: { src: "/manus-storage/itinerary-tea-train-generated-hero_f2423db1.png", alt: "スリランカ高原の茶畑を走る紅茶列車", caption: "紅茶列車は、移動手段ではなく、旅の中心となる景色の時間として組み込みます。" },
    intro: [
    ...
    days: [
      { label: "DAY 1", stay: "キャンディ泊", title: "空港からキャンディへ。列車旅の前に、都市間移動を整える", body: "到着後はキャンディまで移動し、ホテルにチェックインします。紅茶列車を翌日以降に乗るなら、初日は移動と休息を中心にし、夕方に仏歯寺やキャンディ湖周辺を訪れる程度にすると無理がありません。", details: ["空港からキャンディまでの移動は、列車を乗り継ぐより、到着便に合わせて車で進む方が旅程を組みやすい日です。空港での合流、荷物、ホテル到着までを一本化しておけば、翌日の列車乗車に集中できます。", "仏歯寺を訪れる場合は、服装と参拝時の案内を確認します。夜の寺院周辺を楽しんだ後は、早めにホテルへ戻り、翌日の駅への出発時間と荷物を整えましょう。"], images: [{ src: "/manus-storage/itinerary-tea-train-generated-kandy-day1_562fb4c2.png", alt: "夕方のキャンディ仏歯寺", caption: "DAY 1：キャンディでは、仏歯寺を訪れ、翌日の高原・列車旅に備えます。" }], note: "鉄道の予約状況・運行状況は変わるため、乗車前日にも公式情報を確認します。" },
      { label: "DAY 2", stay: "ヌワラエリヤまたはエッラ泊", title: "紅茶列車に乗る日。駅送迎と荷物の扱いを先に決める", body: "朝はホテルから駅へ向かい、旅程に合う区間で紅茶列車に乗ります。座席の有無、列車の時間、降車駅、駅からホテルまでの送迎を事前に決め、列車に乗る時間を景色のために使えるようにします。", details: ["Sri Lanka Railwaysのオンライン予約では、駅・日付・人数を指定して座席を探せます。予約は支払いが完了して確定するため、予約画面だけで安心せず、乗車区間、日時、乗客情報、チケットの受け取り方法を確認してください。", "スーツケースを持って乗る場合は、長い乗車時間と降車駅での移動を想定します。列車を降りる駅で車が待てるようにしておけば、ホームで次の移動を探す必要がなく、雨や暗くなる時間にも対応しやすくなります。"], images: [{ src: "/manus-storage/itinerary-tea-train-generated-hero_f2423db1.png", alt: "スリランカ高原の茶畑を走る紅茶列車", caption: "DAY 2：紅茶列車の乗車時間は、高原の景色を楽しむための主役の時間です。" }], note: "公式の予約・運行情報を確認し、列車が変わった場合にも駅送迎を調整できるようにします。" },
  ...
  women: {
    ...
    hero: { src: "/manus-storage/itinerary-women-generated-sigiriya-hero_d85f3c77.png", alt: "朝の光を受けるスリランカの世界遺産シーギリヤロック", caption: "ホテル・移動・撮影時間に余白を残しながら、文化三角地帯から南部海岸までをつなぐ6日間です。" },
    intro: [
  ...
  yala: {
    ...
    hero: { src: "/manus-storage/itinerary-yala-generated-safari-hero_5df58d29.png", alt: "スリランカの国立公園を走るサファリジープ", caption: "サファリの早朝出発と前後のホテル移動を、ひとつの旅程として整えるモデルコースです。" },
    intro: [
